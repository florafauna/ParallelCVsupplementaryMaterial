## execute with
## mpiexec -np 512 Rscript --vanilla 6-2_sample_var.R > Rout/6-2_sample_var.Rout 2>&1
...
pars <- cbind(pgg[order(pgg$sse), c("range", "nugget")][c(7,15), ],
              rank=c(7,15))

parGrid <- expand.grid(rank=c(7,15), seed=1:100)
...
nol1 <- paste0("RData/", gsub(pattern="^data", "resample2", nol))
save(pgl, file=nol1)

if(comm.rank==0){
    nog <- paste0("RData/", gsub(pattern="^data", "resample2", gsub("-...._relLoc..-..", "", nol)))
    timeAll <- TT1()["secs"]
